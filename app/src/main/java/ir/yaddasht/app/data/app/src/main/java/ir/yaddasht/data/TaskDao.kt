...
interface TaskDao {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertTaskAttachment(a: TaskAttachment): Long

    @Delete
    suspend fun deleteTaskAttachment(a: TaskAttachment)

    @Query("SELECT * FROM task_attachments WHERE taskId = :taskId")
    suspend fun taskAttachmentsByTask(taskId: Long): List<TaskAttachment>

    @Query("SELECT COUNT(*) FROM task_attachments WHERE taskId = :taskId")
    suspend fun taskAttachmentCount(taskId: Long): Int
}
