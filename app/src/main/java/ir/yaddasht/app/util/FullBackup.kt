            ...
            json.optJSONArray("notes")?.let { arr ->
                for (i in 0 until arr.length()) {
                    try {
                        ...
                        val note = ir.yaddasht.app.data.Note(
            ...
            json.optJSONArray("tasks")?.let { arr ->
                for (i in 0 until arr.length()) {
                    try {
                        val o = arr.getJSONObject(i)
                        val priority = try {
                            Priority.valueOf(o.optString("priority", "NORMAL"))
                        } catch (e: Exception) { Priority.NORMAL }
                        val task = Task(
                            title = o.optString("title", ""),
...
// ✅ تضمین می‌کنیم shareBackupFile حتماً وجود دارد
