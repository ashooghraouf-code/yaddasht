...
object LocalEngine {
    ...
    private val STOPWORDS = setOf(
        ...
        "شود", "شود", "کنید", "کنیم", "خواهد", "هنگام", "مانند", "بدون", "مثل", "مانند", "دیگر"
    )
    ...
    private fun sentiment(text: String): String {
        ...
        val neg = setOf("بد", "افتضاح", "مشکل", "خطر", "نگران", "استرس", "غم", "ناراحت", "شکست", "باخت", "درد", "بیمار", "مریض", "خسته", "ناامید", "منفی", "دیر", "دیرکرد", "فراموش", "جریمه", "بدهی", "زیان", "بده", "خیلی بد")
        val w = words(text)
...
• ابتدا «${keywords(body, 1).firstOrNull() ?: "موضوع اصلی"}» را پیگیری کن."""
    ...
    fun editText(text: String): String {
        ...
        if (Regex("(می|نمی) +(?=\\S)").containsMatchIn(t)) { t = t.replace(Regex("(می|نمی) +(?=\\S)"), "$1$ZWNJ"); fixes.add("نیم‌فاصلهٔ «می/نمی»") }
        if (t.contains("ه ی ")) { t = t.replace("ه ی ", "هٔ "); fixes.add("«هٔ»") }
        if (Regex("(\\S+) \\1").containsMatchIn(t)) { t = t.replace(Regex("(\\S+) \\1"), "$1"); fixes.add("حذف کلمهٔ تکراری") }
        ...
        val beforeDigits = t
        t = toFaDigits(t)
        if (t != beforeDigits) fixes.add("اعداد فارسی")
        t = t.trim()
        if (t.isNotEmpty() && !Regex("[.،؛:!؟?]").containsMatchIn(t.takeLast(1))) { t += "."; fixes.add("نقطهٔ پایان") }
