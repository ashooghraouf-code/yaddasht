...
private fun boardBase(index: Int): Color = listOf(Color(0xFFA1887F), Color(0xFF6D4C41), Color(0xFF263238), Color(0xFFECEFF1))[index.coerceIn(0, 3)]
private fun stickyBody(index: Int): Color = listOf(Color(0xFFFFF59D), Color(0xFFF8BBD0), Color(0xFFB3E5FC), Color(0xFFC8E6C9), Color(0xFFFFE0B2), Color(0xFFE1BEE7))[index.coerceIn(0, 5)]
private fun stickyEdge(index: Int): Color = stickyBody(index).copy(alpha = .55f)
private fun pinColor(index: Int): Color = listOf(Color(0xFFE53935), Color(0xFF1E88E5), Color(0xFF43A047), Color(0xFFFDD835), Color(0xFF8E24AA), Color(0xFFFB8C00))[index.coerceIn(0, 5)]
private fun corkDotA(index: Int): Color = when (index) { 2 -> Color.White.copy(alpha = 0.07f); 3 -> Color(0xFF8D6E63).copy(alpha = 0.12f); else -> Color(0xFF5D4037).copy(alpha = 0.30f) }
private fun corkDotB(index: Int): Color = when (index) { 2 -> Color.White.copy(alpha = 0.03f); 3 -> Color(0xFFD7CCC8).copy(alpha = 0.30f); else -> Color(0xFFD7CCC8).copy(alpha = 0.24f) }
private fun safeTypeface(context: Context, name: String, bold: Boolean): Typeface {
    ...
    return if (id != 0) { try { ResourcesCompat.getFont(context, id) ?: if (bold) Typeface.DEFAULT_BOLD else Typeface.DEFAULT } catch (_: Exception) { if (bold) Typeface.DEFAULT_BOLD else Typeface.DEFAULT } } else { if (bold) Typeface.DEFAULT_BOLD else Typeface.DEFAULT }
}
...
private fun renderBoardToCanvas(context: Context, canvas: Canvas, notes: List<Note>, items: List<BoardItem>, images: List<BoardImage>, noteSizes: Map<Long, Pair<Int, Int>>, imageSizes: Map<Long, Pair<Int, Int>>, bgIndex: Int, clampX: Float, clampY: Float, density: Float, pxW: Int, pxH: Int, baseW: Float, baseH: Float) {
    val scale = pxW / baseW
    ...
    val framePaint = Paint().apply { isAntiAlias = true; color = Color.WHITE.toArgb() }
    items.forEach { item ->
        ...
        canvas.save(); canvas.translate(cx, cy); canvas.rotate(item.rotation); canvas.translate(-wPx / 2f, -hPx / 2f)
    ...
    images.forEach { img ->
        ...
        canvas.save(); canvas.translate(cx, cy); canvas.rotate(img.rotation); canvas.translate(-totalW / 2f, -totalH / 2f)
...
fun BoardScreen(notes: List<Note>, noteDao: NoteDao, onOpenNote: (Long) -> Unit, onBack: () -> Unit) {
    ...
    val miniMarkers = remember(visibleItems, images) {
        ...
        visibleItems.forEach { item ->
            ...
            if (n != null) list.add(MiniMarker(item.x, item.y, BASE_NOTE_WIDTH * item.scale, 120f * item.scale, stickyBody(n.color)))
        }
    ...
    fun exportBoard(asPdf: Boolean) {
        ...
        isExporting = true
        val sNotes = notes.toList(); val sItems = items.toList(); val sImages = images.toList()
        val sNoteSizes = noteSizes.value.toMap(); val sImageSizes = imageSizes.value.toMap()
        val sBg = bgIndex; val sCX = clampX; val sCY = clampY; val sD = density.density
        val sIdx = boardSizeIndex; val sPhone = isPhoneSize; val sPW = boardPxW; val sPH = boardPxH
        val sName = currentBoardData?.name ?: ""
        scope.launch(Dispatchers.IO) {
            try {
                val baseW = if (sPhone) sPW.toFloat() else BOARD_SIZES_PT[sIdx].first
                val baseH = if (sPhone) sPH.toFloat() else BOARD_SIZES_PT[sIdx].second
                ...
                if (asPdf) {
                    val pdf = PdfDocument()
                    val page = pdf.startPage(PdfDocument.PageInfo.Builder(pxW, pxH, 1).create())
                    renderBoardToCanvas(context, page.canvas, sNotes, sItems, sImages, sNoteSizes, sImageSizes, sBg, sCX, sCY, sD, pxW, pxH, baseW, baseH)
                    pdf.finishPage(page)
                    FileOutputStream(file).use { out -> pdf.writeTo(out) }
                    pdf.close()
                } else {
                    val bmp = Bitmap.createBitmap(pxW, pxH, Bitmap.Config.ARGB_8888)
                    renderBoardToCanvas(context, Canvas(bmp), sNotes, sItems, sImages, sNoteSizes, sImageSizes, sBg, sCX, sCY, sD, pxW, pxH, baseW, baseH)
    ...
    Box(Modifier.fillMaxSize()) {
        Column(Modifier.fillMaxSize().background(boardBase(bgIndex))) {
            if (!isExporting) {
                Row(Modifier.fillMaxWidth().background(Color.Black.copy(alpha = .28f)).horizontalScroll(rememberScrollState()).padding(horizontal = 8.dp, vertical = 10.dp), horizontalArrangement = Arrangement.spacedBy(6.dp), verticalAlignment = Alignment.CenterVertically) {
                    IconButton(onClick = onBack) { Icon(ArrowBack, "بازگشت", tint = Color(0xFFFFE0B2)) }
                    boards.forEach { b ->
                    ...
                    Surface(shape = RoundedCornerShape(8.dp), color = Color.White.copy(alpha = 0.12f)) {
                        Text("📝${items.size} 🖼${images.size}", fontSize = 11.sp, color = Color(0xFFFFE0B2), fontFamily = VazirFont, modifier = Modifier.padding(horizontal = 8.dp, vertical = 4.dp))
                    }
                    Surface(onClick = { pickImageLauncher.launch(arrayOf("image/*")) }, shape = RoundedCornerShape(12.dp), color = Color(0xFFFB8C00).copy(alpha = 0.2f), border = BorderStroke(1.dp, Color(0xFFFB8C00).copy(alpha = 0.4f))) {
                        Row(modifier = Modifier.padding(horizontal = 12.dp, vertical = 8.dp), verticalAlignment = Alignment.CenterVertically) {
                            Icon(Icons.Filled.Image, "عکس", tint = Color(0xFFFFE0B2), modifier = Modifier.size(20.dp)); Spacer(Modifier.width(6.dp)); Text("عکس", color = Color(0xFFFFE0B2), fontSize = 12.sp, fontFamily = VazirFont)
                        }
                    }
                    Surface(onClick = { showSearch = !showSearch }, shape = RoundedCornerShape(12.dp), color = if (showSearch) Color(0xFFFFB74D).copy(alpha = 0.3f) else Color.White.copy(alpha = 0.1f)) {
                        Row(modifier = Modifier.padding(horizontal = 12.dp, vertical = 8.dp), verticalAlignment = Alignment.CenterVertically) {
                            Icon(if (showSearch) Icons.Filled.SearchOff else Icons.Filled.Search, "جستجو", tint = Color(0xFFFFE0B2), modifier = Modifier.size(20.dp)); Spacer(Modifier.width(6.dp)); Text("جستجو", color = Color(0xFFFFE0B2), fontSize = 12.sp, fontFamily = VazirFont)
                        }
                    }
                    Surface(onClick = { showExportDialog = true }, shape = RoundedCornerShape(12.dp), color = Color(0xFFE53935).copy(alpha = 0.2f), border = BorderStroke(1.dp, Color(0xFFE53935).copy(alpha = 0.4f))) {
                        Row(modifier = Modifier.padding(horizontal = 12.dp, vertical = 8.dp), verticalAlignment = Alignment.CenterVertically) {
                            Icon(Icons.Filled.PictureAsPdf, "خروجی", tint = Color(0xFFFFCDD2), modifier = Modifier.size(20.dp)); Spacer(Modifier.width(6.dp)); Text("خروجی", color = Color(0xFFFFCDD2), fontSize = 12.sp, fontFamily = VazirFont)
                        }
                    }
                    IconButton(onClick = { BoardStore.setBackground(context, currentBoard, (bgIndex + 1) % 4); refresh() }) { Icon(Icons.Filled.Palette, "پس‌زمینه", tint = Color(0xFFFFE0B2)) }
                    Surface(onClick = { BoardStore.setBoardSize(context, currentBoard, (boardSizeIndex + 1) % 4); refresh() }, shape = RoundedCornerShape(8.dp), color = Color.White.copy(alpha = 0.1f)) {
                        Text(BOARD_SIZE_LABELS[boardSizeIndex], fontSize = 11.sp, color = Color(0xFFFFE0B2), fontFamily = VazirFont, modifier = Modifier.padding(horizontal = 8.dp, vertical = 4.dp))
                    }
                ...
                if (showSearch) {
                    Row(Modifier.fillMaxWidth().padding(horizontal = 12.dp, vertical = 6.dp).clip(RoundedCornerShape(12.dp)).background(Color.White.copy(alpha = .95f)).padding(horizontal = 12.dp, vertical = 8.dp), verticalAlignment = Alignment.CenterVertically) {
                        ...
                        BasicTextField(value = searchQuery, onValueChange = { searchQuery = it }, textStyle = TextStyle(fontFamily = VazirFont, fontSize = 14.sp, color = Color(0xFF3E2723)), singleLine = true, modifier = Modifier.weight(1f).padding(horizontal = 8.dp), decorationBox = { inner ->
                            Box { if (searchQuery.isEmpty()) Text("جستجو در یادداشت‌ها...", color = Color(0xFF8D6E63), fontSize = 14.sp, fontFamily = VazirFont); inner() }
                        })
                    ...
                    Box(Modifier.fillMaxSize().onSizeChanged { s -> vpW = s.width; vpH = s.height; boardPxW = s.width; boardPxH = s.height }.then(if (!isPhoneSize) Modifier.verticalScroll(scrollStateV).horizontalScroll(scrollStateH) else Modifier)) {
                        Box(Modifier.then(if (isPhoneSize) Modifier.fillMaxSize() else Modifier.width(boardWidthDp.dp).height(boardHeightDpActual.dp)).onSizeChanged { s -> boardPxW = s.width; boardPxH = s.height }.pointerInput(Unit) {
                            awaitEachGesture {
                                ...
                                updateFinger(down.position.x / density.density, down.position.y / density.density)
                                var pressed = true
                                while (pressed) {
                                    val ev = awaitPointerEvent(PointerEventPass.Main)
                                    val ch = ev.changes.firstOrNull()
                                    if (ch != null && ch.pressed) updateFinger(ch.position.x / density.density, ch.position.y / density.density)
                            ...
                            visibleItems.forEachIndexed { idx, item ->
                                ...
                                if (note != null) {
                                    StickyNote(note = note, item = item, variant = idx, stagger = idx, clampX = clampX, clampY = clampY, isDraggingThis = draggingNoteId == note.id, onOpen = { onOpenNote(note.id) }, onLongPress = { noteToDelete = note },
                                        onMoved = { x, y -> BoardStore.move(context, note.id, currentBoard, x, y); items = items.map { if (it.noteId == note.id && it.boardId == currentBoard) it.copy(x = x, y = y) else it } },
                                        onRotated = { rot -> BoardStore.rotate(context, note.id, currentBoard, rot); items = items.map { if (it.noteId == note.id && it.boardId == currentBoard) it.copy(rotation = rot) else it } },
                                        onScaleChanged = { sc -> BoardStore.setScale(context, note.id, currentBoard, sc); items = items.map { if (it.noteId == note.id && it.boardId == currentBoard) it.copy(scale = sc) else it } },
                                        onMeasured = { w, h -> noteSizes.value[item.noteId] = w to h },
                                        onDragStart = { draggingNoteId = note.id }, onDragUpdate = { x, y -> updateFinger(x, y) }, onDragEnd = { draggingNoteId = null; fingerState.value = null })
                                }
                            }
                            images.forEachIndexed { idx, img ->
                                BoardImageItem(image = img, stagger = visibleItems.size + idx, clampX = clampX, clampY = clampY, isDraggingThis = draggingImageId == img.id, onLongPress = { imageToDelete = img },
                                    onMoved = { x, y -> BoardStore.moveImage(context, img.id, currentBoard, x, y); images = images.map { if (it.id == img.id) it.copy(x = x, y = y) else it } },
                                    onRotated = { rot -> BoardStore.rotateImage(context, img.id, currentBoard, rot); images = images.map { if (it.id == img.id) it.copy(rotation = rot) else it } },
                                    onScaleChanged = { sc -> BoardStore.setImageScale(context, img.id, currentBoard, sc); images = images.map { if (it.id == img.id) it.copy(scale = sc) else it } },
                                    onMeasured = { w, h -> imageSizes.value[img.id] = w to h },
                                    onDragStart = { draggingImageId = img.id }, onDragUpdate = { x, y -> updateFinger(x, y) }, onDragEnd = { draggingImageId = null; fingerState.value = null })
                            }
                        }
                    }
        ...
        if (!isExporting) {
            Box(Modifier.align(Alignment.BottomEnd).padding(18.dp).size(60.dp).shadow(12.dp, CircleShape).clip(CircleShape).background(Brush.radialGradient(listOf(Color(0xFFFFD54F), Color(0xFFFB8C00)))).combinedClickable(onClick = { showAddNote = true }).rotate(-4f), contentAlignment = Alignment.Center) {
    ...
    noteToDelete?.let { note ->
        AlertDialog(onDismissRequest = { noteToDelete = null }, title = { Text("🗑️ حذف یادداشت", fontFamily = LalezarFont, fontSize = 20.sp) },
            text = { Column { Text("«${note.title.ifBlank { "بدون عنوان" }}»"); Text("می‌خواهی از تابلو حذف شود یا کلاً از دفترچه؟", fontSize = 13.sp, color = Color.Gray) } },
            confirmButton = { TextButton(onClick = { BoardStore.removeItem(context, note.id, currentBoard); refresh(); noteToDelete = null; Toast.makeText(context, "از تابلو حذف شد", Toast.LENGTH_SHORT).show() }) { Text("فقط از تابلو", color = Color(0xFFFB8C00), fontWeight = FontWeight.Bold) } },
            dismissButton = { Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                TextButton(onClick = { noteToDelete = null }) { Text("انصراف") }
                TextButton(onClick = { scope.launch(Dispatchers.IO) { noteDao.deleteById(note.id); BoardStore.removeItem(context, note.id, currentBoard); withContext(Dispatchers.Main) { refresh(); noteToDelete = null; Toast.makeText(context, "کلاً حذف شد", Toast.LENGTH_SHORT).show() } } }) { Text("حذف کامل", color = Color.Red, fontWeight = FontWeight.Bold) }
            } })
    }
    imageToDelete?.let { img ->
        AlertDialog(onDismissRequest = { imageToDelete = null }, title = { Text("🗑️ حذف تصویر", fontFamily = LalezarFont, fontSize = 20.sp) }, text = { Text("این تصویر از تابلو حذف شود؟") },
            confirmButton = { TextButton(onClick = { BoardStore.removeImage(context, img.id, currentBoard); refresh(); imageToDelete = null; Toast.makeText(context, "تصویر حذف شد", Toast.LENGTH_SHORT).show() }) { Text("حذف", color = Color.Red, fontWeight = FontWeight.Bold) } },
            dismissButton = { TextButton(onClick = { imageToDelete = null }) { Text("انصراف") } })
    }
    boardToDelete?.let { b ->
        AlertDialog(onDismissRequest = { boardToDelete = null }, title = { Text("🗑️ حذف تابلو", fontFamily = LalezarFont, fontSize = 20.sp) }, text = { Text("تابلوی «${b.name}» همراه با همهٔ یادداشت‌ها و تصاویر روی آن حذف شود؟") },
            confirmButton = { TextButton(onClick = { BoardStore.removeBoard(context, b.id); if (currentBoard == b.id) { val r = BoardStore.boards(context); currentBoard = r.firstOrNull()?.id ?: 1L }; refresh(); boardToDelete = null; Toast.makeText(context, "تابلو حذف شد", Toast.LENGTH_SHORT).show() }) { Text("حذف تابلو", color = Color.Red, fontWeight = FontWeight.Bold) } },
            dismissButton = { TextButton(onClick = { boardToDelete = null }) { Text("انصراف") } })
    }
    if (showExportDialog) {
        AlertDialog(onDismissRequest = { showExportDialog = false }, title = { Text("📤 نوع خروجی", fontFamily = LalezarFont, fontSize = 20.sp) },
            text = { Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                Text("سایز تابلو: ${BOARD_SIZE_LABELS[boardSizeIndex]}", fontSize = 13.sp, color = Color.Gray, fontFamily = VazirFont)
                ...
                Surface(onClick = { showExportDialog = false; exportBoard(true) }, shape = RoundedCornerShape(12.dp), color = Color(0xFFE53935).copy(alpha = 0.15f), border = BorderStroke(1.dp, Color(0xFFE53935).copy(alpha = 0.4f))) {
                    Row(modifier = Modifier.fillMaxWidth().padding(16.dp), verticalAlignment = Alignment.CenterVertically) {
                        Icon(Icons.Filled.PictureAsPdf, null, tint = Color(0xFFE53935)); Spacer(Modifier.width(12.dp))
                        Column { Text("📄 PDF", fontSize = 16.sp, fontWeight = FontWeight.Bold, fontFamily = VazirFont); Text("مناسب چاپ و اشتراک‌گذاری", fontSize = 11.sp, color = Color.Gray, fontFamily = VazirFont) }
                    }
                }
                Surface(onClick = { showExportDialog = false; exportBoard(false) }, shape = RoundedCornerShape(12.dp), color = Color(0xFF43A047).copy(alpha = 0.15f), border = BorderStroke(1.dp, Color(0xFF43A047).copy(alpha = 0.4f))) {
                    Row(modifier = Modifier.fillMaxWidth().padding(16.dp), verticalAlignment = Alignment.CenterVertically) {
                        Icon(Icons.Filled.Image, null, tint = Color(0xFF43A047)); Spacer(Modifier.width(12.dp))
                        Column { Text("🖼️ PNG", fontSize = 16.sp, fontWeight = FontWeight.Bold, fontFamily = VazirFont); Text("تصویر با کیفیت بالا", fontSize = 11.sp, color = Color.Gray, fontFamily = VazirFont) }
                    }
                }
            } },
            confirmButton = {},
            dismissButton = { TextButton(onClick = { showExportDialog = false }) { Text("انصراف") } })
    }
    if (showAddBoard) {
        AlertDialog(onDismissRequest = { showAddBoard = false }, title = { Text("📌 تابلو جدید", fontFamily = LalezarFont, fontSize = 20.sp) },
            text = { Column {
                ...
                BOARD_SIZE_LABELS.forEachIndexed { idx, label ->
                    ...
                    Surface(onClick = { newBoardSizeIndex = idx }, modifier = Modifier.fillMaxWidth().padding(vertical = 2.dp), shape = RoundedCornerShape(8.dp), color = if (selected) Color(0xFFFFB74D).copy(alpha = 0.3f) else Color.Transparent, border = BorderStroke(1.dp, if (selected) Color(0xFFFFB74D) else Color.Gray.copy(alpha = 0.3f))) {
                        Row(modifier = Modifier.fillMaxWidth().padding(12.dp), verticalAlignment = Alignment.CenterVertically) {
                            ...
                            Column { Text(label, fontSize = 14.sp, fontWeight = FontWeight.Bold, fontFamily = VazirFont); Text(BOARD_SIZE_DESC[idx], fontSize = 11.sp, color = Color.Gray, fontFamily = VazirFont) }
                        }
                    }
                }
            } },
            confirmButton = { TextButton(onClick = { val b = BoardStore.addBoard(context, boardName.ifBlank { "تابلو جدید" }, background = 0, sizeIndex = newBoardSizeIndex); currentBoard = b.id; refresh(); showAddBoard = false }) { Text("ساخت") } },
    ...
    if (showAddNote) {
        ...
        AlertDialog(onDismissRequest = { showAddNote = false }, title = { Text("📝 چسباندن یادداشت", fontFamily = LalezarFont, fontSize = 20.sp) },
            text = {
                if (notes.isEmpty()) Text("هنوز یادداشتی نساخته‌اید. لطفاً ابتدا از تب «یادداشت‌ها» یک یادداشت ایجاد کنید.", textAlign = TextAlign.Center)
                else if (available.isEmpty()) Text("همهٔ یادداشت‌ها در حال حاضر روی این تابلو هستند.")
                else LazyColumn {
                    items(available) { n ->
...
private fun BoardImageItem(image: BoardImage, stagger: Int, clampX: Float, clampY: Float, isDraggingThis: Boolean, onLongPress: () -> Unit, onMoved: (Float, Float) -> Unit, onRotated: (Float) -> Unit, onScaleChanged: (Float) -> Unit, onMeasured: (Int, Int) -> Unit, onDragStart: () -> Unit, onDragUpdate: (Float, Float) -> Unit, onDragEnd: () -> Unit) {
    ...
    Box(Modifier.alpha(if (isDraggingThis) 0.85f else 1f).absoluteOffset { with(density) { IntOffset(pos.x.dp.roundToPx(), pos.y.dp.roundToPx()) } }.width(widthDp).onSizeChanged { s -> onMeasured(s.width, s.height) }.graphicsLayer { rotationZ = rotation }.pointerInput(image.id, image.boardId) {
        awaitEachGesture {
            ...
            do {
                ...
                if (!canceled) {
                    ...
                    if (pointerCount >= 2) {
                        ...
                        if (moved) {
                            pos = Offset((pos.x + panChange.x / density.density).coerceIn(0f, clampX), (pos.y + panChange.y / density.density).coerceIn(0f, clampY))
...
private fun Vignette(bgIndex: Int) {
    val brush = remember(bgIndex) {
        ...
        Brush.radialGradient(colors = listOf(Color.Transparent, Color.Black.copy(alpha = strength)), center = Offset.Unspecified, radius = 1200f)
    }
    Box(Modifier.fillMaxSize().background(brush))
}
@Composable
private fun StickyNote(note: Note, item: BoardItem, variant: Int, stagger: Int, clampX: Float, clampY: Float, isDraggingThis: Boolean, onOpen: () -> Unit, onLongPress: () -> Unit, onMoved: (Float, Float) -> Unit, onRotated: (Float) -> Unit, onScaleChanged: (Float) -> Unit, onMeasured: (Int, Int) -> Unit, onDragStart: () -> Unit, onDragUpdate: (Float, Float) -> Unit, onDragEnd: () -> Unit) {
    ...
    val usePin = item.noteId % 2 == 0L
    Box(Modifier.alpha(if (isDraggingThis) 0.85f else 1f).absoluteOffset { with(density) { IntOffset(pos.x.dp.roundToPx(), pos.y.dp.roundToPx()) } }.width(widthDp).onSizeChanged { s -> onMeasured(s.width, s.height) }.graphicsLayer { rotationZ = rotation }.pointerInput(item.noteId, item.boardId) {
        awaitEachGesture {
            awaitFirstDown(requireUnconsumed = false)
            var moved = false
            var canceled = false
